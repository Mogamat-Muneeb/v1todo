...
import { signInWithPopup, GoogleAuthProvider, sendEmailVerification, onAuthStateChanged, signOut } from "firebase/auth";
...
onMounted(() => {
  onAuthStateChanged(auth, (user) => {
    if (user) {
      ...
      const userTodosQuery = query(todosCollection, where("uid", "==", user.uid));
      console.log(userTodosQuery, "userTodosQuery");
...
<template>
  ...
  <div v-if="user">
  <div
    class="sticky top-0 bg-white flex items-center justify-center w-full gap-4 px-3 pt-20 md:px-0 max-h-[500px] overflow-y-auto"
  >
  <button @click="signUserOut()">signUserOut</button>
  <p>
  {{user.displayName}}
